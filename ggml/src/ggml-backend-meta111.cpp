        ...
        if (pipeline_chunks && enable_pipeline_chunks) {
            ...
            if (!threaded_chunk_reduce) {
                ...
                for (const pipeline_trace_event & event : trace_events) {
                    if (strcmp(event.phase, "compute_begin 0") == 0) {
                    ...
                    } else if (strcmp(event.phase, "transfer_ready") == 0) {
                        transfer_ready[event.chunk][event.j_src] = event.time_us;
                    } else if (strcmp(event.phase, "transfer_begin") == 0) {
                    ...
                    } else if (strcmp(event.phase, "transfer_end") == 0 && transfer_start[event.chunk][event.j_src] >= 0) {
                        ...
                        add_start[event.chunk][0] = event.time_us;
                    } else if (strcmp(event.phase, "add_begin 1") == 0) {
                        add_start[event.chunk][1] = event.time_us;
                    } else if (strcmp(event.phase, "add_end 0") == 0 && add_start[event.chunk][0] >= 0) {
                        add_us[0] += event.time_us - add_start[event.chunk][0];
                        total_end = std::max(total_end, event.time_us);
                    } else if (strcmp(event.phase, "add_end 1") == 0 && add_start[event.chunk][1] >= 0) {
                        add_us[1] += event.time_us - add_start[event.chunk][1];
                        total_end = std::max(total_end, event.time_us);
                    }
                }
