...
public:
    ...
    // Non-zero only for the Qwen2 prefill DAG path. The input tensors have
    // token_count rows and are populated from this slice of the macro ubatch.
    uint32_t token_begin = 0;
    uint32_t token_count = 0;
};
...
struct llm_graph_context {
 ...
 std::vector<ggml_tensor *> * down_chunk_outputs = nullptr) const;
    ...
    llm_graph_input_attn_kv * build_attn_inp_kv_range(uint32_t token_begin, uint32_t token_count) const;
