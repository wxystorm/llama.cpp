...
llama_model_qwen2::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = layer_begin; il < layer_end; ++il) {
        if (return_wave_eligible && il == return_wave_first_layer) {
            ...
            // Materialize the final transformer layer outputs after all final
            // split FFNs have been submitted.  There is no next Attention to
            // create these nodes for us, so the normal pending-return barrier
            // before the output tail drains the remaining Phone reductions.
            for (size_t ci = 0; ci < chunk_sizes.size(); ++ci) {
                GGML_ASSERT(pending_wave_down_chunks[ci] != nullptr);
                GGML_ASSERT(pending_wave_residual_chunks[ci] != nullptr);
                ...
                const std::string wave_l_out_name =
                    "prefill_wave_l_out_chunk_" + std::to_string(ci);
                cb(wave_l_out, wave_l_out_name.c_str(), layer_end - 1);
                ggml_build_forward_expand(gf, wave_l_out);
                hidden_chunks[ci] = wave_l_out;
            }
            ...
            // With a single token chunk, cur is the exact same tensor as the
            // terminal prefill_ffn_down_chunk_0-* boundary. Renaming it to
            // l_out-* makes Meta lose the final down-chunk marker, so the
            // prefill norm -> down handoff cannot find its i+1 boundary.
            // With multiple chunks, cur is a distinct CONCAT result and can
            // keep the normal l_out name.
        ...
        const int n_prefill_chunks = planned_chunk_tokens > 0 ?
            ...
            std::min<int64_t>(qwen2_prefill_ffn_chunk_count(), cur->ne[1]);
