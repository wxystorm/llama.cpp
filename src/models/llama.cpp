...
void llama_model_llama::load_arch_tensors(llama_model_loader &) {
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        if (n_expert == 0) {
            ...
            layer.ffn_gate_exps = create_tensor(tn(LLM_TENSOR_FFN_GATE_EXPS, "weight", i), { n_embd, n_ff, n_expert },
                                                TENSOR_NOT_REQUIRED);
            layer.ffn_down_exps =
...
template <bool embed>
llama_model_llama::graph<embed>::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        if (model.layers[il].ffn_gate_inp == nullptr) {
            ...
            const bool                    use_decode_chunked_ffn = !embed && n_tokens == 1 &&
                                                model.split_mode() == LLAMA_SPLIT_MODE_TENSOR &&
                                                hybrid_mode == llama_hybrid_layer_mode::TENSOR_SPLIT && n_chunks >= 1 &&
            ...
            const bool use_prefill_chunked_ffn =
                ...
                hybrid_mode == llama_hybrid_layer_mode::TENSOR_SPLIT && n_prefill_chunks >= 1 && loras->empty() &&
                cvec->tensor_for(il) == nullptr;

            if (use_decode_chunked_ffn) {
                ggml_tensor * ffn_hidden =
                    build_ffn(cur, model.layers[il].ffn_up, model.layers[il].ffn_up_b, model.layers[il].ffn_up_s,
                              model.layers[il].ffn_gate, model.layers[il].ffn_gate_b, model.layers[il].ffn_gate_s,
                              nullptr, nullptr, nullptr, NULL, LLM_FFN_SILU, LLM_FFN_PAR, il);
                ...
                const bool          prepare_next_layer =
                    next_layer != nullptr && model.hybrid_layer_mode(il + 1) != llama_hybrid_layer_mode::PHONE_ONLY;
                ...
                const bool use_partial_qkv = prepare_next_layer && next_layer->wqkv == nullptr &&
                                             can_slice_input(next_layer->wq) && can_slice_input(next_layer->wk) &&
                ...
                for (int i = 0; i < n_chunks; ++i) {
                    ...
                    if (model.layers[il].ffn_down_b != nullptr) {
                        ggml_tensor * bias = ggml_view_1d(ctx0, model.layers[il].ffn_down_b, length,
                                                          offset * model.layers[il].ffn_down_b->nb[0]);
                    ...
                    if (prepare_next_layer) {
                        ggml_tensor * norm_weight = ggml_view_1d(ctx0, model.layers[il + 1].attn_norm, length,
                        ...
                        if (use_partial_qkv) {
                            auto build_partial = [&](ggml_tensor * weight) {
                                ...
                                const size_t view_offs = (offset / bs) * weight->nb[0];

                                ggml_tensor * weight_chunk =
                                    ggml_view_2d(ctx0, weight, length, weight->ne[1], weight->nb[1], view_offs);
                    ...
                    if (use_partial_qkv) {
                        auto finish_projection = [&](ggml_tensor * acc, ggml_tensor * scale, ggml_tensor * bias,
                            ...
                            if (hparams.f_clamp_kqv > 0.0f) {
                                projection = ggml_clamp(ctx0, projection, -hparams.f_clamp_kqv, hparams.f_clamp_kqv);
                            }
                            projection = ggml_reshape_3d(ctx0, projection, n_embd_head, n_heads, n_tokens);
                ...
                for (int i = 0; i < n_prefill_chunks; ++i) {
                    ...
                    ggml_tensor * down_chunk =
                        build_ffn(ffn_norm_chunk, model.layers[il].ffn_up, model.layers[il].ffn_up_b,
                                  model.layers[il].ffn_up_s, model.layers[il].ffn_gate, model.layers[il].ffn_gate_b,
                                  model.layers[il].ffn_gate_s, model.layers[il].ffn_down, model.layers[il].ffn_down_b,
